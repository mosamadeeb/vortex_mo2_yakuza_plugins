...
const GAME_ID = 'yakuza5remastered';
const STEAMAPP_ID = '1105510';
...
const DATA_PATH = path.join('main', 'data');
const MODS_PATH = path.join('main', 'mods');
const EXT_MODS_PATH = '_externalMods'
const GAME_EXE = path.join('main', 'Yakuza5.exe');
...
function main(context) {
    ...
    context.registerInstaller(
        'yakuza5remastered-mod-installer',
        25,
        testMod,
...
function prepareForModding(discovery, api) {
    return checkForRMM(api, path.join(discovery.path, 'main', RMM_EXE));
}
...
function findUnsupportedFiles(files) {
    const ext = ['.usm', '.cpk', '.hca', '.adx']
    return files.filter(file => (file.length > 4 && ext.includes(file.substr(file.length - 4))));
}
